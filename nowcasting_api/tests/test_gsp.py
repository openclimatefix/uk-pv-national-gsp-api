...
async def test_read_latest_gsp_id_greater_than_total(db_session, api_client):
...
async def test_read_latest_gsp_id_equal_to_total(db_session, api_client):
    ...
    response = await api_client.get("/v0/solar/GB/gsp/forecast/317")
...
@freeze_time("2022-01-01")
async def test_read_truths_for_a_specific_gsp(db_session, api_client):
    ...
    response = await api_client.get("/v0/solar/GB/gsp/pvlive/122")
...
async def test_read_pvlive_for_gsp_id_over_total(db_session, api_client):
    """Check solar/GB/gsp/pvlive returns 204 when gsp_id over total"""

    gsp_id = 318
    response = await api_client.get(f"/v0/solar/GB/gsp/pvlive/{gsp_id}")

    assert response.status_code == 204


@freeze_time("2022-01-01")
async def test_read_truths_for_gsp_id_less_than_total(db_session, api_client):
