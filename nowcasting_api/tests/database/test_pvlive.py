...
def test_get_gsp_yield_value_compact(db_session, gsp_yields):
    ...
    assert gsp_yields[0].datetime_utc.isoformat() == datetime(2022, 1, 1).isoformat()
    assert gsp_yields[0].generation_kw_by_gsp_id[122] == 2
    assert gsp_yields[1].datetime_utc.isoformat() == datetime(2022, 1, 1, 12).isoformat()
    assert gsp_yields[1].generation_kw_by_gsp_id[122] == 3
    assert gsp_yields[2].datetime_utc.isoformat() == datetime(2022, 1, 2).isoformat()
